...
type ParsedCareer = Omit<Career, 'id'> & {
    isValid: boolean;
    errors: string[];
};

interface CareerImportDialogProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    onSuccess: () => void;
}

export function CareerImportDialog({ open, onOpenChange, onSuccess }: CareerImportDialogProps) {
    const firestore = useFirestore();
    const { toast } = useToast();
    const [isProcessing, setIsProcessing] = useState(false);
    const [isSaving, setIsSaving] = useState(false);
    const [parsedData, setParsedData] = useState<ParsedCareer[]>([]);
        ...
        try {
            ...
            const processedData = json.map(row => {
                ...
                if (!name) {
                    ...
                    career.errors.push('La columna "name" no puede estar vacía.');
                }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const careersCol = collection(firestore, 'careers');
            
            validData.forEach(career => {
                const newDocRef = doc(careersCol);
                const careerData: Omit<Career, 'id'> = {
                    name: career.name,
                };
                batch.set(newDocRef, careerData);
            });
            ...
            toast({
                ...
                description: `Se han guardado ${validData.length} nuevas carreras.`,
            });
            onSuccess();
        } catch (error) {
             console.error(error);
    ...
    return (
        <Dialog open={open} onOpenChange={(isOpen) => {
            ...
            <DialogContent className="max-w-4xl flex flex-col max-h-[90vh]">
                <DialogHeader>
                    <DialogTitle>Importar Carreras desde Archivo</DialogTitle>
                    <DialogDescription>
                        Sube un archivo Excel (.xlsx, .xls) o CSV con una única columna: <strong>name</strong>.
                    </DialogDescription>
                    <div className="pt-2">
                ...
                <div className="flex-1 overflow-y-auto -mx-6 px-6">
                    <div className="py-4 space-y-6">
                        <div className="flex items-center justify-center w-full px-6">
                            <label htmlFor="dropzone-file-career" className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-lg cursor-pointer bg-muted hover:bg-background/80">
                                ...
                                <Input id="dropzone-file-career" type="file" className="hidden" onChange={handleFileChange} accept=".xlsx, .xls, .csv" />
                            </label>
                        </div>
                ...
                <DialogFooter className="pt-6">
                    ...
                    <Button onClick={handleSave} disabled={isSaving || isProcessing || parsedData.filter(r => r.isValid).length === 0}>
                        ...
                        Guardar {parsedData.filter(r => r.isValid).length} Carreras
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
