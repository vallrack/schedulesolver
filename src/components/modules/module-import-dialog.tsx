...
type ParsedModule = Omit<Module, 'id'> & {
    isValid: boolean;
    errors: string[];
};
...
export function ModuleImportDialog({ open, onOpenChange, onSuccess }: ModuleImportDialogProps) {
    ...
    const handleDownloadTemplate = () => {
        const headers = [['name', 'totalHours', 'description']];
        const exampleData = [
            ['Fundamentos de Programación', 40, 'Introduce los conceptos básicos de la lógica de programación.'],
            ['Bases de Datos', 60, 'Cubre modelado de datos, SQL y bases de datos relacionales.'],
            ['Diseño Web Responsivo', 50, 'Enseña a crear sitios web adaptables con HTML, CSS y JavaScript.'],
        ];
        ...
        XLSX.writeFile(wb, 'plantilla_modulos.xlsx');
    };
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const processedData = json.map(row => {
                ...
                const totalHours = parseInt(String(lowerCaseRow['totalhours'] || 0), 10);
                ...
                if (!name) {
                    ...
                    module.errors.push('La columna "name" no puede estar vacía.');
                }
                
                if (isNaN(totalHours) || totalHours <= 0) {
                    module.isValid = false;
                    module.errors.push('La columna "totalHours" debe ser un número positivo.');
                }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const modulesCol = collection(firestore, 'modules');
            
            validData.forEach(module => {
                const newDocRef = doc(modulesCol);
                const moduleData: Omit<Module, 'id'> = {
                    name: module.name,
                    totalHours: module.totalHours,
                    description: module.description,
                };
                batch.set(newDocRef, moduleData);
            });
            ...
            toast({
                ...
                description: `Se han guardado ${validData.length} nuevos módulos.`,
            });
            onSuccess();
        } catch (error) {
             console.error(error);
    ...
    return (
        <Dialog open={open} onOpenChange={(isOpen) => {
            ...
            <DialogContent className="max-w-4xl flex flex-col max-h-[90vh]">
                <DialogHeader>
                    <DialogTitle>Importar Módulos desde Archivo</DialogTitle>
                    <DialogDescription>
                        Sube un archivo Excel (.xlsx, .xls) o CSV con las columnas: <strong>name</strong>, <strong>totalHours</strong>, y <strong>description</strong> (opcional).
                    </DialogDescription>
                    <div className="pt-2">
                ...
                <div className="flex-1 overflow-y-auto -mx-6 px-6">
                    <div className="py-4 space-y-6">
                        <div className="flex items-center justify-center w-full px-6">
                            <label htmlFor="dropzone-file-module" className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-lg cursor-pointer bg-muted hover:bg-background/80">
                                ...
                                <Input id="dropzone-file-module" type="file" className="hidden" onChange={handleFileChange} accept=".xlsx, .xls, .csv" />
                            </label>
                        </div>
                                    ...
                                    <Table>
                                        <TableHeader>
                                            <TableRow>
                                                ...
                                                <TableHead>totalHours</TableHead>
                ...
                <DialogFooter className="pt-6">
                    ...
                    <Button onClick={handleSave} disabled={isSaving || isProcessing || parsedData.filter(r => r.isValid).length === 0}>
                        ...
                        Guardar {parsedData.filter(r => r.isValid).length} Módulos
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
